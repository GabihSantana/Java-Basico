...
public class EleicaoView {
	...
	protected void createContents() {
			...
			public void widgetSelected(SelectionEvent e) {
				try {
					...
					if (textStatus.getCharCount() == 0) {					
						caixaWarning.setMessage("O status da Eleição deve ser informado");
						caixaWarning.open();
					} else{
						
						LocalDate data = LocalDate.of(dateTimeRealizacao.getYear(),
								dateTimeRealizacao.getMonth() + 1, // SWT trabalha com mes no padrão 0 - 11
			...
			public void widgetSelected(SelectionEvent e) {
				...
				try {
					...
					if (textCodEleicao.getCharCount() == 0) {					
						caixaWarning.setMessage("O campo código eleição está em branco!");
						caixaWarning.open();
					} else{
						...
						dateTimeRealizacao.setMonth(modelEleicao.getDataRealizacao().getMonthValue()-1);// SWT trabalha com mes no padrão 0 - 11
				...
				try {
					...
					if (textCodEleicao.getCharCount() == 0) {					
						...
						if (modelEleicao.getListaVotos().size()==0) {
							caixaWarning.setMessage("A Eleição informada não possui votos");
							caixaWarning.open();
						}else {
		...
		lblTotalDeVotos.setFont(localResourceManager.create(FontDescriptor.createFrom("Segoe UI", 13, SWT.NORMAL)));
		...
		textTotalVotos.setFont(localResourceManager.create(FontDescriptor.createFrom("Segoe UI", 13, SWT.NORMAL)));
